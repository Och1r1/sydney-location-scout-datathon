...
# Paths
# ---------------------------------------------------------------------------
BASE_DIR = Path(__file__).parent / "sydney-location-scout-datathon"
...
def score_locations(
    ...
    if district and district.strip():
        mask = feature_df['sa2_name'].str.contains(district.strip(), case=False, na=False)
        scored_filtered  = scored_df[mask.values]
        feature_filtered = feature_df[mask.values]
        if len(scored_filtered) == 0:
